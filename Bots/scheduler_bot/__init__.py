    ...
    def __init__(self, key,bot_ID,password="rrr", update=False, start_time=8, end_time=20, interval=15,):
        ...
        def process_name_step(message):
            try:
                ...
                markup.row_width = 2
                markup.add(InlineKeyboardButton("Сегодня", callback_data="cb_day_today"),
                           InlineKeyboardButton("Завтра", callback_data="cb_day_tomorrow"))
                markup = self.add_reset(markup)
                msg = self.bot.reply_to(message, 'В какой день удобно ?', reply_markup=markup)
        ...
        def process_day_step(call):
            ...
            markup = self.add_reset(markup)
            self.bot.send_message(chat_id, "В какой час ?", reply_markup=markup)
        ...
        def process_hours_step(call):
            ...
            markup = self.add_reset(markup)
            self.bot.send_message(chat_id, "Минуты ?", reply_markup=markup)
            update_stat(order, call.from_user)
        ...
        def process_minutes_step(call):
            ...
            markup.add(types.KeyboardButton(text="Отправить мой номер", request_contact=True))

            msg = self.bot.send_message(chat_id, "Ваш номер телефона ?", reply_markup=markup)
