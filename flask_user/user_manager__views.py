...
class UserManager__Views(object):
    ...
    def _send_confirm_email_email(self, user, user_email):
        ...
        if self.USER_ENABLE_EMAIL and self.USER_ENABLE_CONFIRM_EMAIL:
            ...
            # Flash a system message
            email = user_email.email if user_email else user.email
            flash(_('A confirmation email has been sent to %(email)s with instructions to complete your registration.',
                    email=email), 'success')
    ...
    def _do_login_user(self, user, safe_next_url, remember_me=False):
        ...
        if self.USER_ENABLE_EMAIL \
            ...
            flash(_(
                'Your email address has not yet been confirmed. Check your email Inbox and Spam folders for the confirmation email or <a href="%(url)s">Re-send confirmation email</a>.',
                url=url), 'error')
            return redirect(url_for('user.login'))
