...
class DynamoDbAdapter(DbAdapterInterface):
    ...
    def find_objects(self, ObjectClass, **kwargs):
        ...
        for field_name, field_value in kwargs.items():
            ...
            if field is None:
                raise KeyError(
                    "DynamoDBAdapter.find_objects(): Class '%s' has no field '%s'." % (ObjectClass, field_name))
    ...
    def find_first_object(self, ObjectClass, **kwargs):
        ...
        for field_name, field_value in kwargs.items():
            ...
            if field is None:
                raise KeyError(
                    "DynamoDBAdapter.find_first_object(): Class '%s' has no field '%s'." % (ObjectClass, field_name))
